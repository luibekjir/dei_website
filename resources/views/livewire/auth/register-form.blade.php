<div class="flex flex-col gap-6">
    <x-auth-header :title="__('Create an account')" :description="__('Complete your details to start exploring and ordering.')" />
    ...
    <form wire:submit="register" class="flex flex-col gap-6">
        ...
        <div class="pt-4 border-t border-zinc-200">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div class="space-y-1.5">
                    <label class="text-sm font-medium text-zinc-800">Provinsi</label>
                    <select wire:model.live="province_id" class="w-full bg-white border border-zinc-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-[#B25C18] outline-none">
                        <option value="">Pilih Provinsi</option>
                        @foreach($provinces as $province)
                            <option value="{{ $province->id }}">{{ $province->name }}</option>
                        @endforeach
                    </select>
                    @error('province_id') <span class="text-xs text-red-500">{{ $message }}</span> @enderror
                </div>

                <!-- City -->
                <div class="space-y-1.5">
                    <label class="text-sm font-medium text-zinc-800">Kota / Kabupaten</label>
                    <select wire:model="city_id" {{ empty($cities) ? 'disabled' : '' }} class="w-full bg-white border border-zinc-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-[#B25C18] outline-none disabled:opacity-50">
            ...
            <div class="mt-4 space-y-2">
                <label class="text-sm font-medium text-zinc-800 flex items-center gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4 text-[#B25C18]">
                ...
                ></div>
                <p class="text-[10px] text-[#AB7B45] italic">Klik pada peta untuk menandai lokasi pengiriman Anda.</p>
        ...
        <div class="flex items-center justify-end mt-4">
            <flux:button type="submit" variant="primary" class="w-full bg-[#B25C18] hover:bg-[#8F4C11] border-none py-6">
                {{ __('Daftar Sekarang') }}
            </flux:button>
        </div>
    </form>
    ...
    <script>
        ...
        // Re-center map when modal/tab changes if needed
        setTimeout(() => {
            map.invalidateSize();
        }, 500);
    </script>
