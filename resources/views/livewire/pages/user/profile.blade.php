<div class="min-h-screen bg-[#FFF8F0] -m-6">
            ...
            <div class="p-4">
                <nav class="space-y-2">
                    ...
                    <button 
                        ...
                        class="w-full text-left px-4 py-3 rounded-lg transition {{ $activeTab === 'business' ? 'bg-orange-500/30 text-zinc-800 font-semibold' : 'text-zinc-700 hover:bg-orange-500/50 hover:text-white' }}"
                    >
                        <div class="flex items-center space-x-3">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @if($activeTab === 'dashboard')
                ...
                <div class="grid grid-cols-3 gap-6">
                    ...
                    <div class="col-span-2 bg-white rounded-lg shadow-sm p-6 flex flex-col">
                        ...
                        <div class="mb-6">
                            <span class="text-4xl font-bold text-[#1D1D1B]">@currency($weeklySpending)</span>
                            <p class="text-zinc-400 text-sm mt-1">Total investment in the last 7 days</p>
                        </div>
                        <div class="relative h-48 flex-1 bg-zinc-50 rounded-2xl flex items-center justify-center border border-dashed border-zinc-200">
                            <p class="text-zinc-400 text-sm">Spending trend will appear here</p>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-lg shadow-sm p-6">
                        <div class="flex flex-col items-center text-center">
                            <div class="w-24 h-24 bg-[#B25C18] rounded-full flex items-center justify-center text-white text-3xl font-bold mb-4 shadow-lg">
                                {{ auth()->user()->initials() }}
                            </div>
                            <h2 class="text-xl font-bold text-zinc-800">{{ $name }}</h2>
                            <p class="text-zinc-500 text-sm mb-6">{{ $email }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-12">
                    <div class="flex items-center justify-between mb-6">
                        <h2 class="text-2xl font-bold text-zinc-800">Saved Addresses</h2>
                    </div>

                    <div class="grid grid-cols-3 gap-6">
                        @forelse($savedAddresses as $address)
                        <div class="bg-zinc-200/50 rounded-lg p-5">
                            <h3 class="text-lg font-semibold text-zinc-800 mb-3">{{ $address->label }}</h3>
                            <p class="text-sm text-zinc-700">{{ $address->address_line }}, {{ $address->city }}</p>
                        </div>
                        @empty
                        <div class="col-span-3 bg-zinc-50 rounded-xl p-10 text-center border border-dashed border-zinc-200">
                            <p class="text-zinc-400">No saved addresses yet.</p>
                        </div>
                        @endforelse

                        <div wire:click="$set('showAddressModal', true); $dispatch('open-address-modal')" class="bg-white rounded-lg border-4 border-dashed border-zinc-200/50 p-5 flex items-center justify-center cursor-pointer hover:border-zinc-300 transition">
                ...
                <!-- Activity History -->
                <div class="mt-12">
                    <h2 class="text-2xl font-bold text-zinc-800 mb-6">Activity History</h2>
                    <div class="space-y-4">
                        @forelse($recentOrders as $order)
                        <div class="bg-zinc-200/50 rounded-lg p-4 flex items-center gap-4">
                            <div class="w-16 h-16 rounded-lg bg-[#F4E6D9] flex items-center justify-center text-[#AB7B45] font-bold text-xl">
                                {{ $order->restaurant->name[0] }}
                            </div>
                            <div class="flex-1">
                                <h3 class="font-semibold text-zinc-800">{{ $order->restaurant->name }}</h3>
                                <p class="text-sm text-zinc-600">{{ $order->created_at->diffForHumans() }}</p>
                            </div>
                        </div>
                        @empty
                        <div class="bg-zinc-50 rounded-xl p-10 text-center border border-dashed border-zinc-200">
                            <p class="text-zinc-400">No activity history found.</p>
                        </div>
                        @endforelse
                    </div>
                </div>
    ...
    <div class="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
        <div class="bg-white rounded-[3rem] w-full max-w-4xl overflow-hidden shadow-2xl">
            <div class="flex h-[80vh]">
                <div class="w-1/2 relative bg-zinc-100">
                    <div id="address-map" class="h-full w-full" wire:ignore></div>
                </div>
                <div class="w-1/2 p-10 overflow-y-auto">
                    <div class="flex justify-between items-center mb-8">
                        <h2 class="text-3xl font-bold text-zinc-800">New Address</h2>
                        <button wire:click="$set('showAddressModal', false)" class="text-zinc-400">✕</button>
                    </div>
                    <form wire:submit.prevent="addAddress" class="space-y-6">
                        <input type="text" wire:model="address_label" placeholder="Label" class="w-full bg-zinc-50 border-zinc-200 rounded-2xl py-4 px-6">
                        <textarea wire:model="address_line" class="w-full bg-zinc-50 border-zinc-200 rounded-2xl py-4 px-6" rows="3"></textarea>
                        <button type="submit" class="w-full bg-[#B25C18] text-white py-5 rounded-2xl font-bold">Save Address</button>
                    </form>
                </div>
            </div>
        </div>
    ...
    <script>
        function initAddressMap() {
            ...
            const map = L.map('address-map').setView([-7.2575, 112.7521], 13);
            ...
            map.on('click', function(e) {
                ...
                fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lng}`)
                    .then(r => r.json()).then(d => {
                        ...
                        @this.set('address_city', d.address.city || d.address.town || '');
                        @this.set('address_province', d.address.state || '');
                    });
            });
        ...
        function initRestaurantMap() {
            ...
            if (!mapContainer || mapContainer._leaflet_id) return;
            const map = L.map('restaurant-map').setView([-7.2575, 112.7521], 13);
            L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png').addTo(map);
            let marker;
            map.on('click', function(e) {
                const { lat, lng } = e.latlng;
